...
{
    readonly List<ExceptionToIgnore> _exceptionsToIgnore = new();
    readonly TimeSpan _delayBetweenAttempts;
    readonly int _maxAttemps;

    public ExceptionIgnorant(int maxAttemps = 5, TimeSpan delayBetweenAttempts = default)
    {
    ...
    public async Task Execute(Func<Task> function, CancellationToken cancellationToken = default)
    {
        var attempt = 0;
